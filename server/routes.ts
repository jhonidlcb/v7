...
import { sendWelcomeEmail, sendContactNotification, sendContactConfirmation, sendPartnerCommissionNotification, sendPaymentProofNotificationToAdmin, sendPaymentProofConfirmationToClient } from "./email";
...
import { z } from "zod";
import { db, users, partners, projects, notifications, tickets, payments, paymentStages, portfolio, referrals, projectMessages, projectFiles, projectTimeline, ticketResponses, paymentMethods, invoices, transactions, budgetNegotiations, twilioConfig, workModalities } from "./db";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/contact", async (req, res) => {
    try {
      ...
      res.json({ 
        ...
        whatsappUrl: "https://wa.me/595985990046?text=Hola,%20he%20realizado%20una%20consulta%20y%20enviado%20los%20detalles%20con%20el%20formulario.%20Me%20gustaría%20obtener%20más%20información."
      });
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Datos inválidos", errors: error.errors });
    ...
    try {
      ...
      for (const admin of adminUsers) {
        await storage.createNotification({
          ...
          message: `El cliente ${client.fullName} confirmó el pago de la etapa "${stage[0].stageName}" mediante ${paymentMethod}. ${proofFile ? 'Comprobante adjunto: ' + proofFile.originalname : 'Sin comprobante adjunto'}.`,
          type: "info",
        });
      }
      ...
      res.json({
        ...updated,
        message: "Comprobante enviado exitosamente. Se han enviado notificaciones por email y puedes contactar por WhatsApp.",
        whatsappReady: true
      });
    } catch (error) {
  ...
  app.post("/api/payment-stages/:id/generate-link", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      // TODO: Integrar nuevo sistema de pagos aquí
      // Por ahora, solo marcamos la etapa como disponible para pago manual
      const updated = await storage.updatePaymentStage(stageId, {
        paymentLink: `#manual-payment-${stageId}`, // Link temporal para pago manual
        status: 'available'
      });
